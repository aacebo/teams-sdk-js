...
export class ForwardClient {
  protected baseUrl = '/me/calendar/calendarView/{event-id}/forward';
  ...
  /**
   * `POST /me/calendar/calendarView/{event-id}/forward`
   *
...
copy of the meeting event. This convenience is not available when forwarding from an Outlook.com account.
   */
  async create(
    body: Endpoints['POST /me/calendar/calendarView/{event-id}/forward']['body'],
    params?: Endpoints['POST /me/calendar/calendarView/{event-id}/forward']['parameters']
  ) {
    ...
    return this.http
      ...
      .then(
        (res) =>
          res.data as Endpoints['POST /me/calendar/calendarView/{event-id}/forward']['response']
      );
  }
}
